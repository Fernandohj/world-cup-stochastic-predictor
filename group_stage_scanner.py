...
print("Iniciando Escáner Matricial (Time Decay + Dixon-Coles HFA)...")

# Cargar ranking FIFA para establecer la fuerza base de cada selección
try:
    ...
    raise Exception("ERROR CRÍTICO: No se encontró '../data/ranking_fifa.csv'.")
...
try:
    ...
    raise Exception("ERROR: No se encontró '../data/results.csv'.")
...
# Aplicar decaimiento exponencial temporal para dar mayor peso a los partidos recientes
...
# Diccionario de Localía Dinámica (HFA)
...
def simular_partido_mundial(equipo_a, equipo_b):
    ...
    print(f"\n--- ESCÁNER EXACTO (DIXON-COLES): {escenario} ---")
    ...
    for m_a, m_b in top_marcadores:
        prob = matriz_prob[m_a, m_b] * 100
        print(f"   {m_a} - {m_b} (Prob: {prob:.1f}%)")
